...
const Pricing = () => {
  return (
    <>
    <div className="min-h-screen bg-[#131B27] font-inter p-6 sm:p-12 flex justify-center">
  <div className="w-full max-w-7xl">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
      {pricingData.map((item) => (
        <div
          key={item.plan}
          className="relative flex flex-col p-6 h-full bg-gray-900 border border-gray-800 rounded-3xl shadow-xl transition-all duration-300 hover:border-[#A146EA]/50"
        >
          {/* Ribbon */}
          {item.isOptimal && (
            <div className="absolute top-0 right-0 transform -translate-y-1/2 translate-x-1/4 z-10">
              <div className="px-4 py-1.5 rounded-full text-xs font-bold text-white bg-gradient-to-r from-[#A146EA] to-[#4B7DDA] shadow-lg shadow-[#A146EA]/30 whitespace-nowrap">
                Best match. Save 40%
              </div>
            </div>
          )}
